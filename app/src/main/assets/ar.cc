...
void AR::loadFromImage(const std::string& path)
{
    ...
    std::string jstr = "{\n"
                       ...
                       "  ]\n"
                       "}";
    target.load(jstr.c_str(), EasyAR::kStorageApp | EasyAR::kStorageJson);
...
{
    ImageTarget target;
    target.load(path.c_str(), EasyAR::kStorageApp, targetname.c_str());
    tracker_.loadTarget(target, new HelloCallBack());
}
...
void ARVideo::openVideoFile(const std::string& path, int texid)
{
    ...
    player_.open(path.c_str(), kStorageApp, callback_);
}
